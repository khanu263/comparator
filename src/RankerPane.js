...
class RankerPane extends React.Component {
  ...
  render() {
    if (this.item.type === "film") {
      return (
        <Box direction="column" justify="start" align={this.props.align}>
          {this.item.poster !== "(no poster found)" && (
            <Image className="poster" src={this.item.poster} margin={{ bottom: "2em" }} />
          )}
          <Heading margin={{ top: "0" }} level="2" textAlign={this.props.align}>
            {this.item.name}
          </Heading>
          <Box direction="column" gap="small" align={this.props.align}>
